...
def normalize(old_name):
    ...
    if not pathlib.Path(old_name).is_dir():
        extension = pathlib.Path(old_name).suffix.lower()
        old_name = old_name.removesuffix(extension)
    ...
    for i in old_name:
        if not i.isdigit() and "a" <= i.lower() <= "z":
            ...
            new_string += "_"
    if pathlib.Path(old_name).is_dir():
        return new_string
    else:
